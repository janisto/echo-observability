...
func filterApplicationFields(fields []zap.Field) []zap.Field {
	...
	for _, field := range fields {
		if field.Type != zapcore.InlineMarshalerType && isReservedApplicationLogField(field.Key) {
			hasReserved = true
			break
		}
	}
...
func isReservedApplicationLogField(key string) bool {
	return key != "error" && isReservedLogField(key)
}
...
type (
	GCPProfileVersion   string
	AWSProfileVersion   string
	AzureProfileVersion string
)
