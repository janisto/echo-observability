...
This Go module is released by manually pushing an annotated `vX.Y.Z` tag for
...
3. confirm that `go.mod` still declares the intended `/v2` module path and the
   documented Go and Echo support lines; and
...
annotated tag at that exact commit, verify it, and push it:
...
The peeled remote annotated tag must still resolve to the reviewed `TARGET`,
and the module must resolve through the public proxy and checksum database from
a fresh module cache. Verify the exact version page at
`https://pkg.go.dev/github.com/janisto/echo-observability/v2@vX.Y.Z`. The Go
