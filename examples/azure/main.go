...
func setup(e *echo.Echo) (*zap.Logger, error) {
	...
	e.Use(
		obs.RequestContext(obs.RequestContextConfig{Logger: logger, Preset: obs.PresetAzure}),
		obs.AccessLogger(obs.AccessLoggerConfig{Logger: logger, Preset: obs.PresetAzure}),
		middleware.Recover(),
	)
...
func main() {
	...
	handler := obs.HTTPRequestContext(obs.HTTPRequestContextConfig{Logger: logger, Preset: obs.PresetAzure})(mux)
	server := &http.Server{
